...
export const getAllBlogs = TryCatch(async (req, res) => {
    ...
    if (searchQuery && category) {
        blogs = await sql `SELECT * FROM blogs WHERE (title ILIKE ${"%" + searchQuery + "%"} )AND category=${category} ORDER BY create_at DESC;`;
    }
    else if (searchQuery) {
        blogs = await sql `SELECT * FROM blogs WHERE title ILIKE ${"%" + searchQuery + "%"} ORDER BY create_at DESC;`;
    }
    else if (category) {
